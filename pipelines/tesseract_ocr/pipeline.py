...
def traitement_page(indice):
    ...
    if(colonne_noire_positions[1]- colonne_noire_positions[0] > 200 * coefficient_larg) :
        ...
        plt.axis('off')
        #plt.show()
        plt.imsave(f"pipelines/tesseract_ocr/pages traitées/{indice}.pdf", image_etendue, cmap='gray')
...
def lecture_ocr(indice):
    ...
    pdf_path = f"pipelines/tesseract_ocr/pages traitées/{indice}.pdf"
...
def traitement_document(nombre_pages):
    ...
    for i in range(nombre_pages):
        fichier_pdf = f"pipelines/tesseract_ocr/pages traitées/{i}.pdf"
        try:
