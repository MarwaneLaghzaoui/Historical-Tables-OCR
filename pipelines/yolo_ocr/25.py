...
def traitement_page(indice,pdf_document):
    ...
    if(colonne_noire_positions[1]- colonne_noire_positions[0] > 200 * coefficient_larg) :
        ...
        plt.axis('off')
        #plt.show()
        plt.imsave(f"pages traitées/{indice}.pdf", image_etendue, cmap='gray')
...
def lecture_ocr(indice):
    ...
    pdf_path = f"pages traitées/{indice}.pdf"
    ...
    with tqdm(total=total, desc="Traitement OCR", unit="cellule") as pbar:
        ...
        excel_path = os.path.join(os.path.dirname(__file__), "resultats.xlsx")
...
def traitement_document(pdf_path):
    ...
    excel_path = os.path.join(BASE_DIR, "resultats.xlsx")
    if os.path.exists(excel_path):
        os.remove(excel_path)

    with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
        df_temp = pd.DataFrame({"Temp": ["Feuille temporaire"]})  # Feuille bidon
        df_temp.to_excel(writer, sheet_name="Temp", index=False)

    for p in range(nombre_pages):
        traitement_page(p,pdf_document)
        lecture_ocr(p)
        colonne_noire_positions = []
        ligne_noire_positions = []
        lignes_noires_inserees = []
    ...
    for i in range(nombre_pages):
        fichier_pdf = f"pages traitées/{i}.pdf"
        try:
